...
public class ExecutorPresetStep : IEquatable<ExecutorPresetStep>
{
    ...
    public void Draw(int i)
    {
        ...
        var childWidth  = availableContent.X - ImGui.GetStyle().ItemSpacing.X;
        var childHeight = availableContent.Y - ImGui.GetStyle().ItemSpacing.Y - (4 * ImGui.GetTextLineHeightWithSpacing());
        
        using (var child = ImRaii.Child("StepContentChild", new(childWidth, childHeight)))
        {
            if (child)
            {
                DrawContentChild();
        ...
        void DrawContentChild()
        {
            ...
            using (var statusCheck = ImRaii.TabItem("状态检查"))
            {
                if (statusCheck)
                {
            ...
            using (var positionCheck = ImRaii.TabItem("坐标"))
            {
                if (positionCheck)
                {
                    ...
                    ImGuiOm.TooltipHover("在执行完左侧的状态检查后, 会按照此处的配置移动到指定的坐标");
            }
